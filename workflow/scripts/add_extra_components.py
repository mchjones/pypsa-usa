...
from _helpers import calculate_annuity, configure_logging, get_snapshots
from add_electricity import add_missing_carriers
from opts._helpers import get_region_buses
from pypsa.descriptors import get_switchable_as_dense as get_as_dense

from build_dlr import (
    get_line_info,
    trunk,
    get_bus_data,
    calc_system_baseline,
    get_relative,
    get_line_dlr_inputs,
    get_line_dlrs_eff,
    gen_year_no_leap,
...
def get_dlr():
    ...
    elif snakemake.wildcards.dlr == "none": # all 1's
    ...
    elif snakemake.wildcards.dlr == "slr": # each column is the discount value
        slr = pd.DataFrame(np.ones_like(all_relative), 
                    index=all_relative.index, 
                    columns=all_relative.columns)
