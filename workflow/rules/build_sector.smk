...
def sector_input_files(wildcards):
    input_files = {
        "network": RESOURCES
        + "{interconnect}/elec_s{simpl}_c{clusters}_ec_l{ll}_{opts}_{dlr}_{gcm_years}.nc",
...
rule add_sectors:
    ...
    log:
        "logs/add_sectors/{interconnect}/elec_s{simpl}_c{clusters}_ec_l{ll}_{opts}_{sector}_{dlr}_{gcm_years}.log",
    group:
        "prepare"
    threads: 1
    resources:
